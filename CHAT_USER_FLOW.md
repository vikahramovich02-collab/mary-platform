...
flowchart TB
    ...
    PICK -->|"Ответы клиентам"| FIRST_AUTO["AU-09 · Собрать первую автоматизацию"]
    PICK -->|"Запись клиентов"| FIRST_AUTO
    PICK -->|"База знаний"| FIRST_AUTO
    FIRST_AUTO --> DRAFT["AU-10 · Черновик схемы"]
    DRAFT --> TEST["AU-11 · Безопасная проверка"]
    TEST --> CONFIRM{"Подтвердить запуск?"}
    CONFIRM -->|"Изменить"| FIRST_AUTO
    CONFIRM -->|"Отложить"| DRAFT
    CONFIRM -->|"Подтвердить"| ACTIVE["AU-16 · Активна"]
```

Первая автоматизация не является скрытым системным шаблоном. Mary собирает её
из подтверждённого контекста бизнеса и выбранного результата. До проверки и
подтверждения она хранится как черновик и не выполняет внешних действий.

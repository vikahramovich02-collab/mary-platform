...
>
> Логический прогон CJM и стресс-веток по этой карте:
...
flowchart TB
    ...
    subgraph ONBOARDING["Первый запуск и основа"]
        ...
        FIRST_DRAFT["AU-09 / AU-10 · Первая автоматизация — черновик"]
        FIRST_TEST["AU-11 · Безопасная проверка"]
        FIRST_CONFIRM{"Подтвердить запуск?"}
        FIRST_ACTIVE["AU-16 · Первая автоматизация активна"]
        FIRST_READY["Платформа готова к первому реальному результату"]
        ...
        ST_PROFILE --> KB_ADD
        ST_PROFILE --> TM_ADD
        IG_CONNECT --> FIRST_DRAFT
        KB_ADD --> FIRST_DRAFT
        TM_ADD --> FIRST_DRAFT
        FIRST_DRAFT --> FIRST_TEST
        FIRST_TEST --> FIRST_CONFIRM
        FIRST_CONFIRM -->|"Изменить"| FIRST_DRAFT
        FIRST_CONFIRM -->|"Отложить"| FIRST_DRAFT
        FIRST_CONFIRM -->|"Подтвердить"| FIRST_ACTIVE
        FIRST_ACTIVE --> FIRST_READY
    end
...
flowchart TB
    ...
    DRAFT["AU-09 / AU-10 · Первая автоматизация — черновик"]
    TEST["AU-11 · Проверить на безопасном примере"]
    REVIEW["Mary показывает схему и результат теста"]
    CONFIRM{"Подтвердить запуск?"}
    ACTIVE["AU-16 · Первая автоматизация активна"]
    READY["Первый реальный запуск"]
    ...
    TEST --> REVIEW
    REVIEW --> CONFIRM
    CONFIRM -->|"Изменить"| DRAFT
    CONFIRM -->|"Отложить"| SAVED["Черновик сохранён"]
    CONFIRM -->|"Подтвердить"| ACTIVE
    SAVED --> DRAFT
    ACTIVE --> READY
    READY --> RESULT["Первое обработанное обращение, запись, ответ или задача"]
```
...
- создавать первую автоматизацию как черновик, связанный с целью onboarding;
- не отправлять сообщения и не изменять production-данные во время проверки;
- не активировать первую автоматизацию без impact preview и подтверждения;
- сохранять черновик, если пользователь откладывает запуск;
